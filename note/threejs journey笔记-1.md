...
2. **配置 Vite**
   ...
   import glsl from 'vite-plugin-glsl'; // 引入插件
   
   export default defineConfig({
     plugins: [glsl()] // 将插件添加到 Vite 的插件数组中
   });
   ```
...
3. **TypeScript 支持（可选）**
   如果你使用 TypeScript，为了在导入 `.glsl` 等文件时获得类型支持，避免 `TS2307` 报错，需要在 `tsconfig.json` 中添加类型声明：
   ...
   或者，你也可以在全局类型声明文件（如 `vite-env.d.ts`）中添加：
...
| 选项                          | 描述                                                  | 默认值                                                       |
| :---------------------------- | :---------------------------------------------------- | :----------------------------------------------------------- |
| **`include`**                 | 指定要作为着色器处理的文件匹配模式（或模式数组）。    | `['**/*.glsl', '**/*.wgsl', '**/*.vert', '**/*.frag', '**/*.vs', '**/*.fs']` |
| **`exclude`**                 | 指定要忽略的文件匹配模式。                            | `undefined`                                                  |
| **`defaultExtension`**        | 当导入语句未指定文件后缀时，自动添加的默认后缀。      | `'glsl'`                                                     |
| **`warnDuplicatedImports`**   | 是否警告重复导入同一着色器代码块。                    | `true`                                                       |
| **`removeDuplicatedImports`** | **v1.3.2+** 是否自动移除已导入的重复代码块。          | `false`                                                      |
| **`importKeyword`**           | **v1.5.0+** 用于引入（include）着色器代码块的关键字。 | `'#include'`                                                 |
| **`minify`**                  | 是否压缩/优化输出的着色器代码。建议在生产环境中启用。 | `false`                                                      |
| **`watch`**                   | 是否监听着色器文件变化并热重载。                      | `true`                                                       |
| **`root`**                    | 配置根目录导入的基准路径。                            | `'/'`                                                        |
...
export default defineConfig({
  plugins: [
    glsl({
      ...
      compress: false, // 注意：v1.4.0后此选项更名为 `minify`
      root: '/'
    })
  ]
});
...
- **导入着色器**：配置完成后，你就可以在 JavaScript 或 TypeScript 代码中直接导入着色器文件了。导入的内容是一个字符串。
  ...
  // 然后就可以在 Three.js, Babylon.js 等库的材质中使用这些字符串
  ```

  

- **代码块复用**：使用 `#include` 指令（默认关键字）可以将其他的着色器代码文件（或"代码块"）内联到主着色器中，非常适合复用公共函数、结构定义等。插件会递归地处理这些 `#include` 语句。

- **与其他插件协作**：如果项目中同时使用了其他 Vite 插件（例如 `vite-plugin-pwa`），并且遇到构建错误，可能需要特别注意插件的配置顺序，或在特定插件的配置中（如 PWA 的 `injectManifest` 配置）再次显式声明 `vite-plugin-glsl` 插件。

- **开发体验**：建议在 VS Code 中安装诸如 **"WebGL GLSL Editor"** 或 **"Shader languages support for VS Code"** 这类插件，它们能提供语法高亮和错误检查，极大提升编写着色器代码的体验。
